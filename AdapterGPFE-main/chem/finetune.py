...
def train(args, model, device, loader, optimizer, prompt):
    ...
    for step, batch in enumerate(loader):
        ...
        __, pred = model(batch.x, batch.edge_index, batch.edge_attr, batch.batch, prompt)
...
def main(runseed, dataset):
    ...
    parser.add_argument('--pnum', type=int, default=3, help='The number of independent basis')
    ...
    # set up model
    model = AdapterGPFE_graphpred(args, args.num_layer, args.emb_dim, num_tasks, JK=args.JK,
                                 drop_ratio=args.dropout_ratio,
                                 graph_pooling=args.graph_pooling, gnn_type=args.gnn_type,
                                 max_bottleneck_dim=args.max_bottleneck_dim,
    ...
    total_params = 0
    for param_group in model_param_group:
